...
/commands...), jamais depuis le passeport (app/passport.py), qui reste une
vue pure sans effet de bord par choix architectural explicite. Un futur
processus de tâches de fond pourra appeler ces mêmes fonctions sur toute la
flotte pour rendre la détection proactive plutôt qu'à la prochaine lecture
— sans rien changer ici (portée à long terme, point 17 de la directive).

Le seuil « depuis combien de temps » est déjà intégré au calcul de l'état
lui-même (STALE_AFTER_INTERVALS pour la communication, UNCONFIRMED_AFTER
pour une commande) : la politique elle-même reste donc volontairement
simple pour la V1 — toute transition confirmée déclenche une alerte de
sévérité "warning", sans hiérarchie de criticité par type d'actif (backlog,
point 9 de la directive).
...
def _command_subject(connection: Connection, point_id: uuid.UUID) -> uuid.UUID:
...
def evaluate_command_timeout(
    ...
    raise_or_repeat_finding(
        ...
        subject_node_id=_command_subject(connection, timed_out["point_id"]),
